<template>
  ...
  <div class="mars3d-btn-wrapper">
    <a-space style="position: absolute; bottom: 7%; right: 20%;z-index: 999">
...
<!--      </mars-button>-->
      ...
      <!-- <mars-button @click="openLink3" style="background-color: white;margin-right: 20px;font-size: 15px;color: #2e333a">
        底图演示
      </mars-button> -->
      <mars-button @click="handleChangeLayer" style="background-color: white;font-size: 15px;color: #2e333a">
        切换底图
      </mars-button>
    </a-space>
  </div>
 ...
 */
import { computed, onUnmounted, onMounted, ref, toRaw } from "vue"
...
// 点击按钮跳转链接
const openLink1 = () => {
  const url = "http://mars3d.cn/editor-vue.html?key=ex_7_12_7&id=graphic/apply/community"
  // 1. 直接跳转
...
const openLink2 = () => {
  const url = "http://mars3d.cn/editor-vue.html?key=ex_1_0_0&id=graphic/entity/billboard-canvas"
  // 1. 直接跳转
  // window.location.href = url
...
const openLink3 = () => {
  const url = "http://localhost:3001/read-vue.html?key=ex_1_0_0&id=graphic/entity/billboard-canvas"
  // 1. 直接跳转
  // window.location.href = url
