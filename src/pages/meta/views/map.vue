...
import RightBar from "@mars/pages/meta/components/RightBar.vue"
import LeftBar from "@mars/pages/meta/components/LeftBar.vue"
...
const marsOnload = (map: any) => {
  ...
  graphicLayer.on(mars3d.EventType.click, function (event: any) {
    ...
    const type = window.polygonType.get(event.graphic.id)
    const graphic: GraphicInterface = store.getters.getGraphicByIdAndType(event.graphic.id, type)
    graphic.highLight()
