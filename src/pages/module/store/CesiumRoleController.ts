 ...
 */
export class CesiumRoleController {

  Cesium: any
  viewer: Cesium.Viewer
  hpRoll: Cesium.HeadingPitchRoll
  position: Cesium.Cartesian3
  speed: number
  rolePrimitive
  boundingSphere
  speedVector
  hpRange: Cesium.HeadingPitchRange
  converter
  cartographic_terrain
  cartographic_3dtiles
  event: boolean
  radian: number
  flag: {
    moveUp: boolean
  ...
  constructor(Cesium: any, viewer: Cesium.Viewer) {
    this.Cesium = Cesium
    this.viewer = viewer
  ...
  move(isUP) {
    ...
    // 判断
    if (mapStore.getters.isPositionInFence(this.position)) {
      message.config({
        maxCount: 1,
        duration: 2
      })
      message.error("警告！人员进入围栏！")
    }
    if (this.cartographic_3dtiles && this.cartographic_3dtiles.height > 0) {
      this.viewer.scene.clampToHeightMostDetailed([this.position]).then((newP) => {
        if (this.Cesium.Cartographic.fromCartesian(newP[0]).height > 0) {
